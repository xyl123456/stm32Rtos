...
void uart_init3(u32 bound){
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority=9 ;//抢占优先级8
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;		//子优先级0
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;			//IRQ通道使能
	NVIC_Init(&NVIC_InitStructure);	//根据指定的参数初始化VIC寄存器
...
extern SemaphoreHandle_t BinarySemaphore_uart3;//串口3二值信号量句柄
...
void USART2_IRQHandler(void)                	//串口2中断服务程序
{
	...
	{
		xSemaphoreGiveFromISR(BinarySemaphore_uart2,&xHigherPriorityTaskWoken);	//释放二值信号量
		portYIELD_FROM_ISR(xHigherPriorityTaskWoken);//如果需要的话进行一次任务切换
	}
}
...
void USART3_IRQHandler(void)                	//串口3中断服务程序
{
	...
	//释放二值信号量
	if((USART3_RX_STA&0x8000)&&(BinarySemaphore_uart3!=NULL))//接收到数据，并且二值信号量有效
	{
		xSemaphoreGiveFromISR(BinarySemaphore_uart3,&xHigherPriorityTaskWoken);	//释放二值信号量
		portYIELD_FROM_ISR(xHigherPriorityTaskWoken);//如果需要的话进行一次任务切换
	}
} 
